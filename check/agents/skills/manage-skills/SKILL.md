---
name: manage-skills
description: 세션 변경사항을 분석하여 검증 스킬 누락을 탐지합니다. 기존 스킬을 동적으로 탐색하고, 새 스킬을 생성하거나 기존 스킬을 업데이트한 뒤 orchestrator.md의 Skills 섹션을 관리합니다.
...
| 스킬 | 설명 | 커버 파일 패턴 |
|------|------|---------------|
| `verify-conventions` | 코딩 컨벤션 및 멱등성 검사를 자동 수행합니다 | `lib/**/*.dart` |
| `verify-build` | Flutter 정적 분석으로 컴파일 에러와 lint 경고를 검출합니다 | `lib/**/*.dart`, `pubspec.yaml` |
| `verify-design-compliance` | Planner 설계서 대비 Generator 구현 충족도를 검증합니다 | `lib/**/*.dart` |
| `verify-architecture` | architecture.md 레이어 규칙과 파일 배치 규칙 준수 여부를 정적으로 검증합니다 | `lib/**/*.dart` |
...
2. **사용자에게 스킬 이름 확인** — `notify_user`를 사용합니다:

   스킬이 커버할 패턴/도메인을 제시하고, 사용자에게 이름을 제공하거나 확인하도록 요청합니다 (`notify_user` 사용).
...
4. **연관 스킬 파일 업데이트** — 새 스킬 생성 후 반드시 아래 3개 파일을 업데이트합니다:
   ...
   **4c. `.agents/rules/orchestrator.md` 업데이트:**
...
- `orchestrator.md`: Skills 테이블 업데이트
...
| `.agents/rules/orchestrator.md` | 프로젝트 지침 (이 스킬이 Skills 섹션을 관리) |
...
6. **orchestrator.md 자체** — `orchestrator.md`의 변경은 문서 업데이트이며, 검증이 필요한 코드 패턴이 아님
