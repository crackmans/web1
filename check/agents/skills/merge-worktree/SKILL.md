...
4. **Fetch latest** (if remote exists): Run `git -C <original-repo-path> fetch origin <target> 2>$null` to ensure target is up to date with remote. Do not fail if no remote.
...
**Create the commit** in the original repo using a temporary text file (Windows/PowerShell Safe):
1. Use the **write_to_file** tool to save your full commit message to a temporary file named `commit_msg.txt` inside `<original-repo-path>`.
2. Run the following command to commit and clean up:
```powershell
git -C <original-repo-path> commit -F commit_msg.txt
Remove-Item <original-repo-path>\commit_msg.txt
```
