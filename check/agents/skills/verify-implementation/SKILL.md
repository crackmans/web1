...
| # | 스킬 | 설명 |
|---|------|------|
| 1 | `verify-conventions` | 코딩 컨벤션 및 멱등성 검사를 자동 수행합니다 |
| 2 | `verify-build` | Flutter 정적 분석(flutter analyze)으로 컴파일 에러와 lint 경고를 검출합니다 |
| 3 | `verify-design-compliance` | Planner 설계서 대비 Generator 구현 충족도를 검증합니다 |
| 4 | `verify-architecture` | architecture.md 레이어 규칙과 파일 배치 규칙 준수 여부를 정적으로 검증합니다 |
...
### Step 4: 동작 모드 분기 (Agent Bypass 제한)

에이전트(Evaluator)가 자율 주행 중 백그라운드로 이 스킬을 호출했다면, 사용자 확인 대기 창(Hang 버그)을 피하기 위해 **즉시 3단계에서 출력된 [오류 체점표 보고서 원문]만 반환하고 스크립트를 즉각 종료(Bypass)**합니다. 즉, 자율 에이전트 모드에서는 5단계의 위험한 스크립트 강제 변경을 절대 이용할 권한이 없습니다 (리스크 통제).

(단, 인간 개발자가 채팅창에서 수동 호출했을 때만 예외적으로 아래의 `notify_user` 메뉴를 노출합니다.)

이슈가 발견된 경우 `notify_user`를 사용하여 단계적으로 사용자에게 확인합니다:
...
각 이슈마다 수정 내용을 보여주고 사용자 승인 여부를 확인합니다.
...
| `.agents/rules/orchestrator.md` | 프로젝트 지침 |
